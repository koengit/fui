...
mapSinkIO, (>!+) :: (b -> IO a) -> Sink a -> Sink b
mapSinkIO f (MkSink h) = MkSink (\x -> do y <- f x; h y)

(>!+) = mapSinkIO

filterSink :: (a -> Bool) -> Sink a -> Sink a
filterSink p (MkSink h) = MkSink (\x -> if p x then h x else return ())

mapMaybeSink, (>?+) :: (b -> Maybe a) -> Sink a -> Sink b
mapMaybeSink f (MkSink h) = MkSink (\x -> case f x of
                                            Nothing -> return ()
                                            Just y  -> h y)
...
foldSource :: b -> (b -> a -> b) -> Source a -> Source b
foldSource x f src = x :> get src (\y src' -> foldSource (f x y) f src')
...
srcA `when` srcE = fst +< (srcA `sync` srcE)

sync :: Source a -> Source b -> Source (a,b)
srcA `sync` srcB = loop (error "sync") (fmap Left srcA >< fmap Right srcB)
 where
  loop x src =
    get src $ \eae src' ->
      case eae of
        Left x' -> loop x' src'
        Right y -> put (x,y) (loop x src')
...
mkGetLast :: Channel a -> a -> IO (IO a)
mkGetLast ch x =
  do ref <- newIORef x
     source ch `flush` MkSink (writeIORef ref)
...
samples :: a -> SampleChannel a -> Source a
samples x ch = samp x (source ch)
 where
  samp x src = get src $ \sx src' ->
                 case sx of
                   New x' -> samp x' src'
                   Sample -> x :> samp x src'

sample :: SampleChannel a -> Sink ()
sample ch = const Sample >+ sink ch
...
(+>!+) :: Source (IO a) -> Sink a -> GUI
src +>!+ snk = action (do flush src $ MkSink (>>= send snk); return empty)
