...
instance Functor Event where
  f `fmap` Event x = Event (f x)

type EventSink a
  = (Event a, Sink a)

newOnceEvent :: IO (EventSink a)
newOnceEvent =
  do var <- newEmptyMVar
     evt <- unsafeInterleaveIO (readMVar var)
     return (evt, MkSink (\x ->
       do tryPutMVar var (Event x)
          return ()
       ))
...
mapMaybeSource :: (a -> Maybe b) -> Source a -> Source b
mapMaybeSource f xs = [ x | Just x <- map f xs ]

flush :: Source a -> Sink a -> IO ()
flush xs snk =
  do forkIO (sequence_ [ send snk x | x <- xs ])
     return ()

flushShow :: Show a => String -> Source a -> Sink a -> IO ()
flushShow lab xs snk =
  do forkIO (sequence_ [ do putStrLn (lab ++ ": " ++ show x); hFlush System.IO.stdout; send snk x | x <- xs ])
     return ()
...
data Channel a
  = MkChannel (MVar (Sink (a,Stream a))) [a]

newChannel :: IO (Channel a)
newChannel =
  do (snk,str) <- newStream
     var       <- newMVar snk
     return (MkChannel var (toList str))
 where
  toList (MkStream (Event (x,str))) = x : toList str
...
run (MkGUI gui) =
  do Gtk.initGUI
     Gtk.timeoutAddFull (threadDelay 0 >> return True) Gtk.priorityDefaultIdle 50
     gui
     Gtk.mainGUI
...
data Canvas
  = MkCanvas Gtk.DrawingArea (MVar Gtk.DrawWindow) (Channel Drawing) (Channel Point)
  ...
  do can <- Gtk.drawingAreaNew
     ...
     can `Gtk.onExpose` \_ ->
       do dw <- Gtk.widgetGetDrawWindow can
          ...
          return True
     let w = MkCanvas can drwVar chaDraw chaMouse
     sequence_ [ prop w | MkProp prop <- props ]
