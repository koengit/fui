...
main =
  do initGUI
     ...
     -- create main window
     win <- windowNew
     set win [ windowTitle := "Bouncing Balls" ]
...
withBalls :: DrawingArea -> IORef [Ball] -> ([Ball] -> IO [Ball]) -> IO ()
...
withBallsEvent :: DrawingArea -> IORef [Ball] -> ([Ball] -> IO [Ball]) -> IO Bool
withBallsEvent can balls f =
  do withBalls can balls f
     return True

new :: DrawingArea -> [Ball] -> IO [Ball]
new can bs =
